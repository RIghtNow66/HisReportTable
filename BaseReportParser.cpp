    ...
    for (const auto& pos : cascadedDirtyCells) {
        ...
        if (isDataMarker(text)) {
            ...
            if (oldMarker.isEmpty()) {
                ...
                task.row = row;
                task.col = col;
                task.queryPath = timeStr;  // 存储时间上下文
                m_queryTasks.append(task);

                newCount++;
                diffInfo.newMarkerCount++;
                qDebug() << QString("新增数据标记：行%1列%2，RTU=%3，时间=%4")
                    .arg(row).arg(col).arg(rtuId).arg(timeStr);
            }
                ...
                if (oldTimestamp > 0) {
                    ...
                    modifiedMarker.rtuId = oldMarker;  // 旧RTU
                    modifiedMarker.oldTimestamp = oldTimestamp;
                    ...
                    if (!newTimeStr.isEmpty()) {
                        ...
                        if (newDateTime.isValid()) {
                            ...
                            modifiedMarker.newTimestamp = oldTimestamp;  // 时间未变
                        }
                    } else {
                        modifiedMarker.newTimestamp = oldTimestamp;
                    }
                ...
                // 移除旧记录
                m_dataMarkerCells.removeAll({ row, col, oldMarker });
